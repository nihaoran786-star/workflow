...
const getDuomiKeys = () => {
    return {
        apiKey: localStorage.getItem('duomi_api_key'),
        secretKey: localStorage.getItem('duomi_secret_key')
    };
};
...
// --- Crypto Helper for Duomi API ---
async function hmacSha256(key: string, message: string): Promise<string> {
    const enc = new TextEncoder();
    const algorithm = { name: "HMAC", hash: "SHA-256" };
    const keyBuf = await window.crypto.subtle.importKey("raw", enc.encode(key), algorithm, false, ["sign", "verify"]);
    const signature = await window.crypto.subtle.sign(algorithm.name, keyBuf, enc.encode(message));
...
// Helper: Center crop/pad image to match target aspect ratio
const fitImageToAspectRatio = async (base64Str: string, targetRatioStr: string): Promise<string> => {
    if (!targetRatioStr) return base64Str;
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            // If ratios are close enough, return original
            ...
            if (sourceRatio > targetRatio) {
                ...
                offsetX = 0;
                offsetY = (img.height - cropH) / 2;
            }

            // Set canvas to the cropped size (or scaled down if too huge)
            canvas.width = cropW;
            canvas.height = cropH;
...
export const generateImageFromText = async (
    ...
    try {
        ...
        // Concurrent but Staggered Execution to prevent 429/Safety Filters
        // If we fire 4 requests instantly, Gemini API might drop them.
        const promises = [];
        for (let i = 0; i < count; i++) {
            promises.push(
                new Promise<string[]>(async (resolve) => {
                    // Stagger by 350ms per request
                    if (i > 0) await wait(i * 350);
                    
                    try {
                        const res = await retryWithBackoff(generateOne);
                        resolve(res);
                    } catch (err) {
                        console.warn(`Image generation attempt ${i + 1} failed:`, err);
                        // Resolve empty so Promise.all doesn't fail everything
                        resolve([]); 
                    }
                })
            );
...
// --- Duomi / Sora-2 External API Implementation ---
const generateDuomiSoraVideo = async (
    prompt: string, 
    aspectRatio: string = "16:9"
): Promise<{ uri: string, isFallbackImage: boolean, videoMetadata: any, uris: string[] }> => {
    const { apiKey, secretKey } = getDuomiKeys();
    
    if (!apiKey || !secretKey) {
        throw new Error("Sora-2 API Key or Secret Key is missing. Please configure them in Settings.");
    }

    const timestamp = Math.floor(Date.now() / 1000);
    const signString = `key=${apiKey}&timestamp=${timestamp}`;
    const signature = await hmacSha256(secretKey, signString);
    ...
    const createRes = await fetch('https://duomiapi.com/v1/videos/generations', {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
            prompt: prompt,
    ...
    if (!createRes.ok) {
        ...
        throw new Error(`Sora-2 API Error (${createRes.status}): ${errText}`);
    }

    const createData = await createRes.json();
    const taskId = createData.id || createData.task_id; // Flexible ID handling

    if (!taskId) {
        throw new Error("Sora-2 API did not return a valid Task ID.");
    }
    ...
    while (attempts < maxAttempts) {
        ...
        const pollRes = await fetch(`https://duomiapi.com/v1/videos/generations/${taskId}`, {
            method: 'GET',
            headers: headers
        });

        if (!pollRes.ok) {
             console.warn(`Sora-2 Poll Error: ${pollRes.status}`);
             continue;
        }
        ...
        if (status === 'SUCCEEDED' || status === 'COMPLETED' || status === 'SUCCESS') {
            // Assume result structure, commonly in 'video', 'output', 'url' or 'generations'
            videoUrl = pollData.video?.url || pollData.url || pollData.output?.[0] || pollData.result?.url;
            if (!videoUrl && pollData.videos && pollData.videos.length > 0) {
                 videoUrl = pollData.videos[0].url;
            }
            break;
        } else if (status === 'FAILED' || status === 'ERROR') {
            throw new Error(`Sora-2 Generation Failed: ${pollData.error || 'Unknown error'}`);
        }
    }
    ...
    return {
        uri: videoUrl,
        uris: [videoUrl],
        videoMetadata: { model: 'sora-2', id: taskId },
...
export const generateVideo = async (
    ...
    // --- Sora-2 Route ---
    if (model === 'sora-2') {
        return retryWithBackoff(() => generateDuomiSoraVideo(prompt, options.aspectRatio));
    }

    // --- Wan 2.1 (Pollo) Route ---
    if (model.includes('wan')) {
        // Implementation for Wan via Pollo would go here
        // Currently placeholder logic as per previous implementation requirements
    }
    ...
    // CRITICAL: Force input image to match requested aspect ratio to prevent Veo from defaulting to input ratio
    let finalInputImageBase64: string | null = null;
    if (inputImageBase64) {
        try {
            // Pre-process crop to match aspect ratio if provided
            let processedImage = inputImageBase64;
            if (options.aspectRatio) {
                processedImage = await fitImageToAspectRatio(inputImageBase64, options.aspectRatio);
            }

            const compat = await convertImageToCompatibleFormat(processedImage);
