...
import { 
    ...
    Minus, FolderHeart, Unplug, Sparkles, ChevronLeft, ChevronRight, Scan, Music, Mic2, Loader2, Check
} from 'lucide-react';
...
const ExpandedView = ({ media, onClose, onSetMain }: { media: any, onClose: () => void, onSetMain: (id: string, src: string) => void }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center transition-all duration-500 ease-[${SPRING}] ${visible ? 'bg-black/90 backdrop-blur-xl' : 'bg-transparent pointer-events-none opacity-0'}`} onClick={handleClose}>
             <div className={`relative w-full h-full flex items-center justify-center p-8 transition-all duration-500 ease-[${SPRING}] ${visible ? 'scale-100 opacity-100' : 'scale-90 opacity-0'}`} onClick={e => e.stopPropagation()}>
                ...
                {hasMultiple && (
                    <button 
                        onClick={(e) => { e.stopPropagation(); onSetMain(media.nodeId, media.images[currentIndex]); }}
                        className={`absolute top-6 right-16 px-4 py-2 rounded-full backdrop-blur-md border transition-all text-xs font-bold flex items-center gap-2 z-[110]
                            ${media.src === media.images[currentIndex] 
                                ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50 cursor-default shadow-[0_0_15px_rgba(6,182,212,0.3)]' 
                                : 'bg-white/10 text-white hover:bg-white/20 border-white/10'}`}
                    >
                        {media.src === media.images[currentIndex] ? <Check size={14} /> : <LayoutTemplate size={14} />}
                        {media.src === media.images[currentIndex] ? '当前首图' : '设为首图'}
                    </button>
                )}
...
export const App = () => {
  ...
  const [connectionStart, setConnectionStart] = useState<{ id: string, x: number, y: number, type?: 'input' | 'output' } | null>(null);
  ...
  const handleSetMainImage = (nodeId: string, src: string) => {
      setNodes(prev => prev.map(n => n.id === nodeId ? { ...n, data: { ...n.data, image: src } } : n));
      // Update the currently open expanded view to reflect the new main image
      setExpandedMedia(prev => prev ? { ...prev, src: src } : null);
  };
  
  // ... (rest of helper functions: getApproxNodeHeight, getNodeBounds, getNodeNameCN, getNodeIcon, handleFitView, saveHistory, undo, deleteNodes, addNode, handleAssetGenerated, handleSketchResult, handleMultiFrameGenerate, handleWheel, handleCanvasMouseDown, handleGlobalMouseMove, handleGlobalMouseUp, handleNodeUpdate, handleReplaceFile, handleNodeAction, saveCurrentAsWorkflow, saveGroupAsWorkflow, loadWorkflow, deleteWorkflow, renameWorkflow, keyboard shortcuts, handleCanvasDragOver, handleCanvasDrop) ...
  ...
  const addNode = useCallback((type: NodeType, x?: number, y?: number, initialData?: any, connectionFrom?: string) => {
      ...
      const defaults: any = { 
          ...
          // Set Default Aspect Ratio for Video nodes to prevent 'undefined' state
          aspectRatio: type === NodeType.VIDEO_GENERATOR ? '16:9' : undefined,
          ...initialData
      };
      ...
      const newNode: AppNode = {
        ...
        data: defaults, 
        inputs: connectionFrom ? [connectionFrom] : []
      };
      
      setNodes(prev => [...prev, newNode]); 
      if (connectionFrom) {
          setConnections(prev => [...prev, { from: connectionFrom, to: newNode.id }]);
      }
  ...
  const handleGlobalMouseUp = useCallback((e: MouseEvent) => {
      ...
      // RESTORED: Drag to Connect Logic (Drop on canvas to create node)
      if (connectionStartRef.current) {
          // If we are here, it means we didn't drop on a port (which stops prop), so we dropped on empty space.
          setContextMenu({
              visible: true,
              x: e.clientX,
              y: e.clientY,
          ...
          setContextMenuTarget({
              type: 'create',
              fromConnection: connectionStartRef.current.id // Pass the source node ID to the new node creator
          });
          setConnectionStart(null); // Clear the active connection line
          return;
      }
  ...
  return (
    <div className="w-screen h-screen overflow-hidden bg-transparent">
      ...
      >
          ...
          <div className="absolute inset-0 pointer-events-none opacity-[0.15]" style={{ backgroundImage: 'radial-gradient(circle, rgba(67, 56, 202, 0.5) 1px, transparent 1px)', backgroundSize: `${32 * scale}px ${32 * scale}px`, backgroundPosition: `${pan.x}px ${pan.y}px` }} />
          ...
          <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-700 ease-[${SPRING}] z-50 pointer-events-none ${nodes.length > 0 ? 'opacity-0 scale-105' : 'opacity-100 scale-100'}`}>
                {/* Simplified Welcome / Quick Start */}
                <div className="flex items-center gap-2 mb-6 text-cyan-200/60 text-xs font-medium tracking-wide">
                    <div className="px-1.5 py-0.5 rounded-md bg-cyan-900/30 border border-cyan-500/30 text-[10px] flex items-center gap-1">
                        <MousePointerClick size={10} />
                        <span>Double Click</span>
                    </div>
                    <span>to create node on canvas</span>
                </div>

                <div className={`flex items-center gap-1.5 p-1.5 rounded-[18px] bg-[#09090b]/40 border border-white/10 backdrop-blur-xl shadow-2xl ${nodes.length > 0 ? 'pointer-events-none' : 'pointer-events-auto'}`}>
                    <button onClick={() => addNode(NodeType.IMAGE_GENERATOR)} className="flex items-center gap-2.5 px-5 py-3 rounded-[14px] bg-white/5 hover:bg-white/10 text-zinc-300 hover:text-white transition-all border border-white/5 hover:border-white/20 group shadow-sm hover:shadow-lg hover:-translate-y-0.5 duration-300">
                        <ImageIcon size={16} className="text-cyan-500 transition-colors group-hover:text-cyan-300" />
                        <span className="text-[13px] font-medium tracking-wide">文字生图</span>
                    </button>
                    
                     <button onClick={() => addNode(NodeType.VIDEO_GENERATOR)} className="flex items-center gap-2.5 px-5 py-3 rounded-[14px] bg-white/5 hover:bg-white/10 text-zinc-300 hover:text-white transition-all border border-white/5 hover:border-white/20 group shadow-sm hover:shadow-lg hover:-translate-y-0.5 duration-300">
                        <Film size={16} className="text-purple-500 transition-colors group-hover:text-purple-300" />
                        <span className="text-[13px] font-medium tracking-wide">文生视频</span>
                    </button>

                    <button onClick={() => addNode(NodeType.VIDEO_GENERATOR, undefined, undefined, { generationMode: 'FIRST_LAST_FRAME' })} className="flex items-center gap-2.5 px-5 py-3 rounded-[14px] bg-white/5 hover:bg-white/10 text-zinc-300 hover:text-white transition-all border border-white/5 hover:border-white/20 group shadow-sm hover:shadow-lg hover:-translate-y-0.5 duration-300">
                        <Link size={16} className="text-emerald-500 transition-colors group-hover:text-emerald-300" />
          ...
          <div style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})`, width: '100%', height: '100%', transformOrigin: '0 0' }} className="w-full h-full">
              ...
              <svg className="absolute top-0 left-0 w-full h-full overflow-visible pointer-events-none z-0" xmlns="http://www.w3.org/2000/svg" style={{ overflow: 'visible', pointerEvents: 'none', zIndex: 0 }}>
                  ...
                  {connectionStart && (() => {
                      ...
                      if (connectionStart.id === 'smart-sequence-dock') {
                          ...
                          // Adjust start position based on type (input/output)
                          if (connectionStart.type === 'input') {
                              startX = startNode.x - 3; // Left side
                          } else {
                              startX = startNode.x + (startNode.width || 420) + 3; // Right side
                          }
              ...
              <Node
                  ...
                  onPortMouseDown={(e, id, type) => { e.stopPropagation(); setConnectionStart({ id, x: e.clientX, y: e.clientY, type }); }}
              ...
              <div className="fixed z-[100] bg-[#1c1c1e]/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-1.5 min-w-[160px] animate-in fade-in zoom-in-95 duration-200 origin-top-left" style={{ top: contextMenu.y, left: contextMenu.x }} onMouseDown={(e) => e.stopPropagation()}>
                  ...
                  {contextMenuTarget?.type === 'create' && (
                      <>
                          ...
                          {[NodeType.PROMPT_INPUT, NodeType.IMAGE_GENERATOR, NodeType.VIDEO_GENERATOR, NodeType.AUDIO_GENERATOR, NodeType.VIDEO_ANALYZER, NodeType.IMAGE_EDITOR].map(t => { const ItemIcon = getNodeIcon(t); return ( <button key={t} className="w-full text-left px-3 py-2 text-xs font-medium text-slate-200 hover:bg-white/10 rounded-lg flex items-center gap-2.5 transition-colors" onClick={() => { addNode(t, (contextMenu.x-pan.x)/scale, (contextMenu.y-pan.y)/scale, undefined, contextMenuTarget?.fromConnection); setContextMenu(null); }}> <ItemIcon size={12} className="text-cyan-400" /> {getNodeNameCN(t)} </button> ); })}
                      </>
                  )}
          ...
          <ExpandedView media={expandedMedia} onClose={() => setExpandedMedia(null)} onSetMain={handleSetMainImage} />
          ...
          <SmartSequenceDock 
             ...
             onConnectStart={(e, type) => { e.preventDefault(); e.stopPropagation(); setConnectionStart({ id: 'smart-sequence-dock', x: e.clientX, y: e.clientY, type: type }); }}
          />
          <SonicStudio 
