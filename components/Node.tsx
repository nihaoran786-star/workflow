...
interface NodeProps {
  ...
  onExpand?: (data: { type: 'image' | 'video', src: string, rect: DOMRect, images?: string[], initialIndex?: number, nodeId?: string }) => void;
...
const NodeComponent: React.FC<NodeProps> = ({ 
  ...
  const handleMouseEnter = () => {
    ...
    // Don't auto-show grid on hover for folded stack design, user clicks to expand
  ...
  const handleExpand = (e: React.MouseEvent) => { 
      ...
      if (onExpand && mediaRef.current) { 
          ...
          if (node.type.includes('IMAGE') && node.data.image) {
              // Pass full list if stacked
              const images = node.data.images && node.data.images.length > 0 ? node.data.images : [node.data.image];
              const currentIndex = images.indexOf(node.data.image);
              onExpand({ type: 'image', src: node.data.image, rect, images: images, initialIndex: currentIndex >= 0 ? currentIndex : 0, nodeId: node.id }); 
              ...
              onExpand({ type: 'video', src: src, rect, images: videos, initialIndex: safeIndex, nodeId: node.id }); 
          }
      }
  };
  ...
  const renderMediaContent = () => {
      ...
      const isMultiImage = node.data.images && node.data.images.length > 1;

      return (
        <div className="w-full h-full relative group/media overflow-hidden bg-zinc-900" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onClick={isMultiImage ? handleExpand : undefined}>
            {!hasContent ? (
                ...
                <>
                    {/* Multi-Image Stack Effect - IMPROVED for Shadow Bug */}
                    {isMultiImage && (
                        <>
                             {/* Third Card (Back) */}
                            <div className="absolute top-4 left-4 right-4 bottom-8 bg-zinc-800 rounded-lg border border-white/5 transform rotate-[-4deg] scale-[0.92] z-0 opacity-100 shadow-lg pointer-events-none" />
                            {/* Second Card (Middle) */}
                            <div className="absolute top-2 left-2 right-2 bottom-5 bg-zinc-800 rounded-lg border border-white/5 transform rotate-[-2deg] scale-[0.96] z-0 opacity-100 shadow-lg pointer-events-none" />
                            
                            {/* Badge */}
                            <div className="absolute top-3 right-3 z-20 bg-black/80 backdrop-blur-md px-2 py-0.5 rounded-full border border-white/10 flex items-center gap-1 shadow-sm">
                                <Layers size={10} className="text-cyan-400" />
                                <span className="text-[9px] font-bold text-white">{node.data.images?.length}</span>
                            </div>
                        </>
                    )}

                    {node.data.image ? 
                        // Image - Ensure relative z-10 to stay on top of stack
                        <img ref={mediaRef as any} src={node.data.image} className={`w-full h-full object-cover relative z-10 transition-transform duration-700 bg-zinc-900 ${isMultiImage ? 'cursor-pointer hover:scale-[1.02]' : 'group-hover/media:scale-105'}`} draggable={false} onContextMenu={(e) => onMediaContextMenu?.(e, node.id, 'image', node.data.image!)} /> 
                    : 
                        <SecureVideo 
                            videoRef={mediaRef} // Pass Ref to Video
                            src={node.data.videoUri} 
                            className="w-full h-full object-cover bg-zinc-900 relative z-10" 
                            loop 
                            muted 
