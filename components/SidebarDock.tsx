...
import { 
    ...
    Clapperboard, Mic2, Settings, FlaskConical, PlayCircle, Music2
} from 'lucide-react';
...
export const SidebarDock: React.FC<SidebarDockProps> = ({
    ...
    const [activePanel, setActivePanel] = useState<'history' | 'workflow' | 'add' | 'beta' | null>(null);
    ...
    const handleSidebarHover = (id: string) => {
        if (['add', 'history', 'workflow', 'beta'].includes(id)) {
            if (closeTimeoutRef.current) clearTimeout(closeTimeoutRef.current);
            setActivePanel(id as any);
        } else {
            // Close panel if hovering over non-panel items (undo/chat)
                ...
                <>
                    ...
                    <div className="flex-1 p-3 space-y-2">
                        <button 
                            ...
                            className={`w-full text-left p-3 rounded-xl bg-white/5 hover:bg-white/10 flex items-center gap-3 text-sm transition-colors border border-transparent hover:border-white/5 hover:shadow-lg ${isMultiFrameOpen ? 'border-cyan-500/50 bg-cyan-900/10' : ''}`}
                        >
                            <div className="p-2 bg-gradient-to-br from-purple-500/20 to-blue-500/20 rounded-lg text-cyan-200">
                                <Clapperboard size={18} />
                            </div>
                            <div className="flex flex-col">
                                <span className="font-bold text-xs text-slate-200">智能多帧 (Smart Seq)</span>
                                <span className="text-[10px] text-slate-500">视频合成与运镜控制</span>
                            </div>
                        </button>
                        
                        <button 
                            onClick={(e) => { e.stopPropagation(); if(onToggleSonicStudio) onToggleSonicStudio(); setActivePanel(null); }} 
                            className={`w-full text-left p-3 rounded-xl bg-white/5 hover:bg-white/10 flex items-center gap-3 text-sm transition-colors border border-transparent hover:border-white/5 hover:shadow-lg ${isSonicStudioOpen ? 'border-cyan-500/50 bg-cyan-900/10' : ''}`}
                        >
                            <div className="p-2 bg-gradient-to-br from-pink-500/20 to-orange-500/20 rounded-lg text-pink-200">
                                <Mic2 size={18} />
                            </div>
                            <div className="flex flex-col">
                                <span className="font-bold text-xs text-slate-200">音频中心 (Sonic Studio)</span>
                                <span className="text-[10px] text-slate-500">配音、音效与实时对话</span>
                            </div>
                        </button>
                    </div>
                </>
                    ...
                    {[NodeType.PROMPT_INPUT, NodeType.IMAGE_GENERATOR, NodeType.VIDEO_GENERATOR, NodeType.AUDIO_GENERATOR, NodeType.VIDEO_ANALYZER, NodeType.IMAGE_EDITOR].map(t => {
                        ...
                        return (
                            <button 
                                ...
                                className="w-full text-left p-3 rounded-xl bg-white/5 hover:bg-white/10 flex items-center gap-3 text-sm text-slate-200 transition-colors border border-transparent hover:border-white/5 hover:shadow-lg group"
                            >
                                <div className="p-2 bg-white/10 group-hover:bg-cyan-500/20 rounded-lg text-slate-300 group-hover:text-cyan-200 shadow-inner transition-colors">
                                    <ItemIcon size={16} />
                                </div> 
                                <div className="flex flex-col">
                                    <span className="font-medium text-xs group-hover:text-white transition-colors">{getNodeNameCN(t)}</span>
                                </div>
                            </button>
                        );
                    })}
    ...
    return (
        <>
            {/* Left Vertical Dock */}
            <div 
                className="fixed left-6 top-1/2 -translate-y-1/2 flex flex-col items-center gap-4 py-6 px-3 bg-[#18181b]/60 backdrop-blur-2xl border border-white/5 rounded-2xl shadow-2xl z-50 animate-in slide-in-from-left-10 duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]"
                onMouseLeave={handleSidebarLeave}
            >
                {[
                    { id: 'add', icon: Plus },
                    { id: 'workflow', icon: FolderHeart }, 
                    { id: 'beta', icon: FlaskConical, active: isMultiFrameOpen || isSonicStudioOpen }, // Combined Beta
                    ...
                    <div key={item.id} className="relative group">
                        <button 
                            ...
                            className={`relative w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 ${activePanel === item.id || item.active ? 'text-cyan-400 bg-white/10 scale-110' : 'text-slate-400 hover:text-white hover:scale-110'}`}
                        >
                            <item.icon size={22} strokeWidth={1.5} />
                            {/* Dot Indicator for Active State */}
                            {(activePanel === item.id || item.active) && <div className="absolute -right-1 top-1 w-1.5 h-1.5 bg-cyan-400 rounded-full shadow-[0_0_8px_rgba(34,211,238,0.8)]"></div>}
                        </button>
                    </div>
                ))}
                ...
                <button 
                    ...
                    className="relative group w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 text-slate-400 hover:text-white hover:scale-110"
                >
